...
func (s *SetIntLogRecord) Redo(tx *Transaction) (bool, error) {
	...
	err = tx.SetInt(s.block, s.offset, s.newValue, false)
	if err != nil {
		return false, err
	}
...
// Page format: [op(4)] [txNum(8)] [lsn(8)] [prevLSN(8)] [filename(4+len(filename))] [blockNum(4)] [offset(4)] [oldvalue(4)] [newvalue(4)] [crc32(4)]
