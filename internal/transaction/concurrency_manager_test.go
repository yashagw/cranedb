...
func TestConcurrencyManager_MVCCLocking(t *testing.T) {
	...
	// Test 1: xLock works for write protection
	err := cm1.xLock(block)
	require.NoError(t, err)
	assert.True(t, lockTable.HasXLock(block))

	// Test 2: Idempotent — acquiring same exclusive lock again should work
	err = cm1.xLock(block)
	require.NoError(t, err)
	assert.True(t, lockTable.HasXLock(block))

	// Test 3: Another manager's xLock should block while cm1 holds it
